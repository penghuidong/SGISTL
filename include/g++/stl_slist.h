...
__slist_make_link(__slist_node_base* prev_node, __slist_node_base* new_node)
{
...
inline void 
__slist_splice_after(__slist_node_base* pos, __slist_node_base* before_first, __slist_node_base* before_last)
{
...
inline __slist_node_base* 
__slist_reverse(__slist_node_base* node)
{
...
private:
	...
	static list_node* create_node(const value_type& x) {
		...
		try{
			construct(&node->data, x);  //new (p) T1(value);
			node->next = 0;
		}
...
public:
	...
	iterator insert(iterator pos) {
		return iterator(_insert_after(__slist_previous(&head, pos.node), value_type()));
	}
...
public:
  ...
  // Linear in distance(begin(), pos), and linear in L.size().
	void splice(iterator pos, slist& L) {
		if (L.head.next)
			__slist_splice_after(__slist_previous(&head, pos.node), &L.head, __slist_previous(&L.head, 0));
	}
...
slist<T, Alloc>& 
slist<T,Alloc>::operator=(const slist<T, Alloc>& L){
	if (&L != this) {
...
void slist<T,Alloc>::merge(slist<T,Alloc>& L, StrictWeakOrdering comp)
{
	list_node_base* n1 = &head;
	while (n1->next && L.head.next) {
		if (comp(((list_node*) L.head.next)->data,((list_node*) n1->next)->data))
			__slist_splice_after(n1, &L.head, L.head.next);
		n1 = n1->next;
	}
	if (L.head.next) {
