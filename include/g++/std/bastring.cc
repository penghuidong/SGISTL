...
create(size_t extra)
{
	extra = frob_size(extra + 1); // 向16取倍数
	Rep *p = new (extra) Rep;
	p->res = extra;
	p->ref = 1;
	p->selfish = false;  // 这个selfish是干嘛用的
	return p;
}
...
clone()
{
	...
	p->copy(0, data(), len);  //创建一个对象并拷贝一份数据
	p->len = len;
	return p->data();
}
...
basic_string <charT, traits, Allocator>::
replace(size_type pos1, size_type n1, const basic_string& str, size_type pos2, size_type n2)
{
...
find (const charT* s, size_type pos, size_type n) const
{
	...
	for (; xpos + n <= length (); ++xpos)
		if (traits::eq(data()[xpos], *s) && traits::compare(data()+xpos, s, n)== 0) return xpos;
	return npos;
}
...
{
	for (; xpo<len; ++xpos)
		if (traits::eq(ptr[xpos], c)) return xpos;
	return npos;
}
...
rfind (charT c, size_type pos) const
{
	...
	if (xpos > pos) xpos = pos;
	for (++xpos; xpos-- > 0; )
		if (traits::eq(data()[xpos], c)) return xpos;
	return npos;
}
...
find_first_of (const charT* s, size_type pos, size_type n) const
{
	...
	for (; xpos<length(); ++xpos)
		if (_find(s, data()[xpos], 0, n)!= npos) return xpos;
	return npos;
}
...
find_first_not_of (charT c, size_type pos) const
{
	size_t xpos = pos;
	for (; xpos < length (); ++xpos)
		if (traits::ne (data () [xpos], c)) return xpos;
	return npos;
}
