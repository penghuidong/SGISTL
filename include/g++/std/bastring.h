...
private:
	struct Rep 
	{
		size_t len, res, ref; // len以使用的长度， res可使用长度
		bool selfish;
...
public:
	const charT* data() const { return rep()->data(); }
	
	size_type length() const{ return rep()->len; }
	
	size_type size() const{ return rep()->len; }
	
	size_type capacity() const { return rep()->res; }
	
	size_type max_size() const{ return (npos - 1)/sizeof (charT); }		// XXX
...
// _lib.string.cons_ construct/copy/destroy:
    ...
    basic_string(InputIterator begin, InputIterator end) : dat(nilRep.grab()) 
	{ 
		assign(begin, end); 
	}
#else
	basic_string(const_iterator begin, const_iterator end): dat(nilRep.grab()) 
	{ 
		assign(begin, end); 
	}
#endif
	//destructor
	~basic_string (){ rep()->release (); }
...
public:
	...
	reference operator[](size_type pos){ selfish(); return (*rep ())[pos]; }
...
public:
	...
	void resize(size_type n){ resize (n, eos()); }
