...
struct __deque_iterator 
{
	...
	void set_node(map_pointer new_node)    //进入new_node缓冲区，但未设置cur
	{
		node = new_node;
		first = *new_node;
...
template <class T, class Alloc = alloc, size_t BufSiz = 0> // 0代表使用512字节缓冲区，非零值代表缓冲区可以存取的元素数量
class deque 
{
public:                         // Basic types
	typedef T 												value_type;
...
public:                         // push_* and pop_*
	...
	void push_back(const value_type& t) {
		if (finish.cur != finish.last - 1) {//最后缓冲区还有2个及以上元素的剩余空间
		  construct(finish.cur, t);
		  ++finish.cur;
		}
		else      //只有当最后一个缓冲区只剩一个备用元素空间时调用push_back_aux，最后一个元素在push_back_aux中构造
		  push_back_aux(t);
	}
...
public:                         // Erase
	iterator erase(iterator pos) 
	{
		iterator next = pos;
		++next;
		difference_type index = pos - start;   //用要删除元素在整个deque中的下标判断前移还是后移
		if (index < (size() >> 1)) {  //前面元素少
...
deque<T, Alloc, BufSize>::erase(iterator first, iterator last)  //左闭右开
{ 
...
void deque<T, Alloc, BufSize>::create_map_and_nodes(size_type num_elements) 
{
  ...
  map_size = max(initial_map_size(), num_nodes + 2);      //最少为8，最大为所需缓冲区数量加二
  map = map_allocator::allocate(map_size);

  map_pointer nstart = map + (map_size - num_nodes) / 2;  //犹如正态分布，头尾两端留出扩充空间
  ...
  finish.cur = finish.first + num_elements % buffer_size();  //指向最后缓冲区最后结点的下一个位置
}
...
void deque<T, Alloc, BufSize>::fill_initialize(size_type n, const value_type& value) 
{
  ...
  try{
    ...
    uninitialized_fill(finish.first, finish.cur, value);//最后一个缓冲区含有未使用缓冲，只需构造前面的
  }
  catch(...) {
...
// Called only if finish.cur == finish.last - 1 when call push_back.
template <class T, class Alloc, size_t BufSize>
...
void deque<T, Alloc, BufSize>::reallocate_map(size_type nodes_to_add, bool add_at_front) 
{
	...
	if (map_size > 2 * new_num_nodes) {  //原配置的map_size数量远大于新map结点数量，在原来map中进行挪动，不配置新map
		new_nstart = map + (map_size - new_num_nodes) / 2 + (add_at_front ? nodes_to_add : 0);
		if (new_nstart < start.node)
			copy(start.node, finish.node + 1, new_nstart);
		else
			copy_backward(start.node, finish.node + 1, new_nstart + old_num_nodes);
	}else {  //原配置map_size不够用，需要配置新的map
